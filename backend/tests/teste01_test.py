...
# Importamos a classe Member do seu arquivo original (assumindo que seja 'models')
from models import Member

# --- Fixture para MOCK do objeto Member retornado pelo MongoEngine ---
# Esta fixture simula um objeto Member do banco de dados com dados criptografados/hashed

@pytest.fixture
def mock_member_instance():
    # Simulando um membro real com email CRIPTOGRAFADO e senha HASHED
    hashed_password = bcrypt.hashpw('senha_secreta'.encode('utf-8'), bcrypt.gensalt()).decode('utf-8')
    
    # Criamos um mock que simula o objeto Member retornado pelo MongoEngine
    member_mock = MagicMock(spec=Member)
    member_mock.email = 'encrypted_test@example.com' # Email CRIPTOGRAFADO
    member_mock.password = hashed_password
    member_mock.id = '60c72b2f9f1b2c3d4e5f6g7h'
    member_mock.first_name = 'Test'
    
    # Adicionamos um método exclude para simular a exclusão da senha
...
# --- MOCKS essenciais (simulando decrypt e Member.objects) ---

# Mockamos a função decrypt para retornar sempre o valor esperado
@pytest.fixture(autouse=True)
def mock_decrypt(mocker):
    # Definimos um side_effect para que 'decrypt' retorne o email 'limpo'
    def fake_decrypt(encrypted_email):
        # Apenas para fins de teste, retornamos o email 'limpo'
...
# Mockamos o Member.objects() para simular o que o MongoEngine retornaria
@pytest.fixture(autouse=True)
def mock_member_objects(mocker, mock_member_instance):
    # Quando Member.objects() é chamado, ele deve retornar uma lista de mocks de membros
    
    # Simulamos o Member.objects().first() ou Member.objects().exclude().first()
    objects_mock = MagicMock()
    objects_mock.objects.return_value = [mock_member_instance] # Lista de membros para iteração
    
    # Configuramos o .first() para retornar a instância mockada
    # Também simulamos a busca por ID
    ...
    def objects_side_effect(**kwargs):
        ...
        # Simula a chamada .exclude() do MongoEngine
...
def test_verify_email_success(mock_member_instance):
    ...
    # O email 'test@example.com' é o email 'limpo' que o mock_decrypt retornará
    result = Member.verify_email('test@example.com')
    assert result is True

def test_verify_email_failure():
    """Verifica se retorna False para um email inexistente."""
...
## 5. Teste de Recuperação de Membro por ID e Email
...
def test_get_member_by_email_excludes_password():
    """Verifica se retorna o membro pelo email (limpo) e exclui a senha."""
    # O email 'test@example.com' é o email 'limpo' que o mock_decrypt retornará
...
def test_get_member_by_email_not_found():
    """Verifica se retorna None para email não encontrado."""
